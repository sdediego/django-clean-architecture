...
@pytest.mark.unit
def test_currency_controller_list_fetch_data(currency):
    num_of_currencies = random.randint(1, 5)
    currency_interator = Mock()
    currency_interator.get_availables.return_value = []
    currency_interator.bulk_save.return_value = None
...
@pytest.mark.unit
def test_exchange_rate_controller_convert_fetch_data(exchange_rate):
    params = {
        'source_currency': exchange_rate.source_currency,
        'exchanged_currency': exchange_rate.exchanged_currency,
        'amount': round(random.uniform(1, 100), 2)
    ...
    assert status == HTTPStatus.OK.value
    assert data['exchanged_currency'] == params['exchanged_currency']
    assert data['exchanged_amount'] == exchange_rate.calculate_amount(
        params.get('amount'))
    assert data['rate_value'] == exchange_rate.rate_value
...
@pytest.mark.unit
def test_exchange_rate_controller_list_fetch_data(exchange_rate):
    series_length = random.randint(1, 10)
    params = {
        'source_currency': exchange_rate.source_currency,
        'exchanged_currency': exchange_rate.exchanged_currency,
...
@pytest.mark.unit
def test_exchange_rate_controller_calculate_twr_fetch_data(exchange_rate):
    num_of_rates = random.randint(1, 10)
    params = {
        'source_currency': exchange_rate.source_currency,
        'exchanged_currency': exchange_rate.exchanged_currency,
